...
def calculate_performance_vs_historical_average(team_score: float, team_scores: List[float]) -> float:
...
def get_injury_bye_factor(lineup: List[Player], max_roster_size: Optional[int] = None) -> float:
...
def get_optimal_vs_actual_factor(league: League, team_lineup: List[Player], opp_lineup: List[Player], actual_outcome: str) -> float:
...
def get_optimal_vs_optimal_factor(league: League, team_lineup: List[Player], opp_lineup: List[Player], actual_outcome: str) -> float:
...
def get_weekly_luck_index(league: League, team: Team, week: int, box_scores: Optional[List[BoxScore]] = None, return_factors: bool = False) -> Union[float, Dict[str, float]]:
